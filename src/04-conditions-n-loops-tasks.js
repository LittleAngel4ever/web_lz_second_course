...
function doRectanglesOverlap(rectangle1, rectangle2) {
  return !(
...
function isCreditCardNumber(CCN) {
  ...
  const sum = digits.reduce((acc, digit, idx) => {
    const processed = idx % 2 === 1 ? digit * 2 : digit;
    const adjusted = processed > 9 ? processed - 9 : processed;
    return acc + adjusted;
  }, 0);
  return sum % 10 === 0;
}
...
function isBracketsBalanced(string) {
  ...
  string.split('').forEach((char) => {
    if ('([{<'.includes(char)) {
      ...
      if (last !== pairs[char]) stack.push('invalid'); // помечаем как ошибку
    }
  });
  return !stack.includes('invalid') && stack.length === 0;
}
 ...
 *
 * @param {number} number
 * @param {number} num, radix of the result
 * @return {string}
 *
 * @example:
 *   1024, 2  => '10000000000'
...
function toNaryString(number, num) {
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < splitPaths[0].length; i += 1) {
    ...
    const isCommon = splitPaths.every((path) => path[i] === segment);
    if (isCommon) {
      common += `${segment}/`;
    } else {
      break;
...
function evaluateTicTacToePosition(position) {
  ...
  const xWins = lines.find((line) => line.every((cell) => cell === 'X'));
  const oWins = lines.find((line) => line.every((cell) => cell === '0'));
  if (xWins) return 'X';
  if (oWins) return '0';
  return undefined;
}
